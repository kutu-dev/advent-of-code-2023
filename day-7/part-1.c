...
hand create_hand(char *input_line)
{
    ...
    // Check if hand is Five of a kind or a Four of a kind
    int num_of_times_first_char = 0;
    for (size_t i = 0; i < 5; i++)
    {
        if (new_hand.cards[i] == new_hand.cards[0])
        {
            num_of_times_first_char++;
        }
    }

    if (num_of_times_first_char == 5)
    {
        new_hand.type = 6;
        return new_hand;
    }
    else if (num_of_times_first_char == 4)
    {
        new_hand.type = 5;
        return new_hand;
    }
    ...
    for (size_t i = 0; i < 5; i++)
    {
        if (new_hand.cards[i] == new_hand.cards[1])
        {
            num_of_times_second_char++;
        }
    }

    if (num_of_times_second_char == 4)
    {
        new_hand.type = 5;
        return new_hand;
    }

    // Check if hand is Full house or Three of a kind

    int num_of_times_third_char = 0;
    for (size_t i = 0; i < 5; i++)
    {
        if (new_hand.cards[i] == new_hand.cards[2])
        {
    ...
    for (size_t i = 0; i < 5; i++)
    {
        if (new_hand.cards[i] == new_hand.cards[3])
        {
            num_of_times_fourth_char++;
        }
    }

    bool there_is_any_three = num_of_times_first_char == 3 || num_of_times_second_char == 3 || num_of_times_third_char == 3 || num_of_times_fourth_char == 3;
    bool there_is_any_two = num_of_times_first_char == 2 || num_of_times_second_char == 2 || num_of_times_third_char == 2 || num_of_times_fourth_char == 2;

    if (there_is_any_three && there_is_any_two)
    {
        new_hand.type = 4;
        return new_hand;
    }
    else if (there_is_any_three)
    {
        new_hand.type = 3;
        return new_hand;
    }

    // Check if hand is Two pair or One pair

    num_of_times_first_char = fmin(2, num_of_times_first_char);
    num_of_times_second_char = fmin(2, num_of_times_second_char);
    num_of_times_third_char = fmin(2, num_of_times_third_char);
    num_of_times_fourth_char = fmin(2, num_of_times_fourth_char);
    
    int sum_of_first_four_chars = num_of_times_first_char + num_of_times_second_char + num_of_times_third_char + num_of_times_fourth_char;

    if (sum_of_first_four_chars == 8 || sum_of_first_four_chars == 7)
    {
        new_hand.type = 2;
    }

    if (sum_of_first_four_chars == 6 || sum_of_first_four_chars == 5)
    {
        new_hand.type = 1;
    }
