...
int get_matched(char **scratchpads, int card_num, int lines) {
    if (card_num > lines) {
        return 0;
    }

    puts(scratchpads[card_num]);

    char *cloned_scratchpad_card = malloc(strlen(scratchpads[card_num]) * sizeof(char));
    strncpy(cloned_scratchpad_card, scratchpads[card_num], strlen(scratchpads[card_num]) * sizeof(char));
    strtok(cloned_scratchpad_card, ":");
    ...
    int accumulative_cards = 0;

    for (size_t j = num_of_matched_numbers; j > 0; j--) {
        printf("COPY %ld NEWPOS %ld\n", j, card_num + j + 1);
        accumulative_cards += get_matched(scratchpads, card_num + j, lines);
        printf("NEW ACCUMULATIVE %d\n", accumulative_cards);
    }

    return 1 + accumulative_cards;
    free(cloned_scratchpad_card);
}
...
int main() {
    ...
    for (size_t i = 0; i < sizeof scratchpads / sizeof scratchpads[0]; i++) {
        printf("REAL %ld\n", i+1);
        sum_of_matched_numbers += get_matched(scratchpads, i, lines);
    }
